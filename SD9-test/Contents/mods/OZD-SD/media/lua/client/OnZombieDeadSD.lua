...
local function zPerks(player, tierzone, zombieSprinterValue)
    if zombieSprinterValue < 1 then return end
    local primaryHandItem = player:getPrimaryHandItem()
    if player:isSeatedInVehicle() or (primaryHandItem and primaryHandItem:IsWeapon() and primaryHandItem:isRanged()) then return end
    local pXP = player:getXp()
    pXP:AddXP(Perks.Strength, tierzone * 10)
    pXP:AddXP(Perks.Fitness, tierzone * 10)
    pXP:AddXP(Perks.Sprinting, tierzone)
    pXP:AddXP(Perks.Nimble, tierzone * 0.1)
end
...
local function itemdrop(item, player, zombie, shouldDrop)
	...
	MDZ_OnCreate_MeleeWeaponVariance(newItem, "OZD")
	if shouldDrop and ZombRand(4) == 0 then
		...
		weaponModData.MaxHitCount = newItem:getMaxHitCount() + 1
		...
		if rng == 0 then
			...
			newItem:setCriticalChance(weaponModData.CriticalChance * weaponModData.soulForgeCritRate)
			newItem:setMinDamage(weaponModData.MinDamage * weaponModData.soulForgeMinDmgMulti)
		...
		elseif rng == 1 then
			...
			newItem:setCriticalChance(weaponModData.CriticalChance * weaponModData.soulForgeCritRate)
			newItem:setMinDamage(weaponModData.MaxDamage * weaponModData.soulForgeMaxDmgMulti)
		...
		elseif rng == 2 then
			...
			newItem:setCriticalChance(weaponModData.CriticalChance * weaponModData.soulForgeCritRate)
			newItem:setCritDmgMultiplier(weaponModData.CritDmgMultiplier * weaponModData.soulForgeCritMulti)
...
local function syncSandbox()
	table1 = splitString(SandboxVars.OZD.table1)
	n1 = #table1
	t1 = ZombRand(n1)+1

	table2 = splitString(SandboxVars.OZD.table2)
	n2 = #table2
	t2 = ZombRand(n2)+1

	table3 = splitString(SandboxVars.OZD.table3)
	n3 = #table3
	t3 = ZombRand(n3)+1
	
	table4 = splitString(SandboxVars.OZD.table4)
	n4 = #table4
	t4 = ZombRand(n4)+1
	
	table5 = splitString(SandboxVars.OZD.table5)
	n5 = #table5
	t5 = ZombRand(n5)+1
	
	rmwtable4 = splitString(SandboxVars.RWC.table4)
	rmwn4 = #rmwtable4 
	rmwt4 = ZombRand(rmwn4)+1	
	
	rmwtable5 = splitString(SandboxVars.RWC.table5)
	rmwn5 = #rmwtable5 
	rmwt5 = ZombRand(rmwn5)+1	
end
...
function OnZombieDeadItemDrop(zombie)
	...
	local cappedRoll = { 100, 250, 400, 550, 700, 850 }
	...
	local t3roll = ZombRand(math.max(SandboxVars.OZD.roll3 - math.floor(zombieSprinterValue*0.75) - controlMod - toxicMod - luckValue, cappedRoll[3] ))
	local t4roll = ZombRand(math.max(SandboxVars.OZD.roll4 - math.floor(zombieSprinterValue*1) - controlMod - toxicMod - luckValue, cappedRoll[4] ))
	local t5roll = ZombRand(math.max(SandboxVars.OZD.roll5 - math.floor(zombieSprinterValue*1.25) - controlMod - toxicMod - luckValue, cappedRoll[5] ))
	local t6roll = ZombRand(math.max(SandboxVars.OZD.roll6 - math.floor(zombieSprinterValue*1.5) - controlMod - toxicMod - luckValue, cappedRoll[6] ))
