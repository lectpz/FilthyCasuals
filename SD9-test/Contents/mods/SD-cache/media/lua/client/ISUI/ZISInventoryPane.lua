...
function ISInventoryPane:transferItemsByWeight(items, container)
	...
	for _,item in ipairs(items) do
		...
		elseif container:getType() == "floor" then
			...
			if item and iFT ~= "Base.WeaponCache" and  iFT ~= "Base.MechanicCache" and iFT ~= "Base.MetalworkCache" and iFT ~= "Base.MedicalCache" and iFT ~= "Base.AmmoCache" and iFT ~= 'Base.ArmorCachePatriot' and iFT ~= 'Base.ArmorCacheDefender' and iFT ~= 'Base.ArmorCacheVanguard' and iFT ~= 'Base.SpiffoCache' then --SD
			...
			if item.items then
				for k, v in ipairs(item.items) do
					if k ~= 1 and v:getContainer() ~= playerInv then
						...
						if v and vFT ~= "Base.WeaponCache" and  vFT ~= "Base.MechanicCache" and vFT ~= "Base.MetalworkCache" and vFT ~= "Base.MedicalCache" and vFT ~= "Base.AmmoCache" and vFT ~= 'Base.ArmorCachePatriot' and vFT ~= 'Base.ArmorCacheDefender' and vFT ~= 'Base.ArmorCacheVanguard' and vFT ~= 'Base.SpiffoCache' then --SD
...
--	end
    ...
    elseif self.inventory == getSpecificPlayer(self.player):getInventory() then
		...
		local iFT = item:getFullType()
		if iFT == 'Base.WeaponCache' or iFT == 'Base.MedicalCache' or iFT == 'Base.MechanicCache' or iFT == 'Base.MetalworkCache' or iFT == 'Base.AmmoCache' or iFT == 'Base.ArmorCachePatriot' or iFT == 'Base.ArmorCacheDefender' or iFT == 'Base.ArmorCacheVanguard' or iFT == 'Base.SpiffoCache' then
			return
		else
			if count == 1 then
