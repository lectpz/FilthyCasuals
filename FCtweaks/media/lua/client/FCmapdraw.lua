...
local alpha = 0.1
local colorTier2 = Color.new(0, 0, 1, alpha)  -- Blue
local colorTier3 = Color.new(1, 0.7, 0, alpha)  -- Orange
local colorTier4 = Color.new(1, 0, 0, alpha)  -- Red

-- assign colors to tiers. Last in first out, otherwise you will skip nested tiers.
function getColorForTier(tier)
	if tier == zonetier[4] then
		return colorTier4
	elseif tier == zonetier[3] then
		return colorTier3
	elseif tier == zonetier[2] then
		return colorTier2
	end
end
...
function drawHatchedRectangleForZone(self, zoneName, spacingFactor, hatchSpacing, api)
	...
	if zoneCoordinates then
		for i = 1, #zoneCoordinates do
			...
			drawHatchedRectangle(self, startX, startY, endX, endY, spacingFactor, hatchSpacing, color, mapAPI)
		end
	end
end
...
function drawHatchedRectangle(self, x1, y1, x2, y2, spacingFactor, hatchSpacing, color, mapAPI)
	-- Draw the main rectangle
	local sides = {{x1, y1, x2, y1}, {x2, y1, x2, y2}, {x2, y2, x1, y2}, {x1, y2, x1, y1}}

	for _, side in ipairs(sides) do
		drawMapLine(self, side[1], side[2], side[3], side[4], color, mapAPI)
	end
...
function drawMapLine(self, x1, y1, x2, y2, color, mapAPI)
	...
	for i = 0, numSymbols do
		...
		addMapSymbol(symbolX, symbolY, color, mapAPI)
	end
...
function addMapSymbol(worldX, worldY, color, mapAPI)
	...
	textureSymbol:setRGBA(color:getR(), color:getG(), color:getB(), alpha)
	textureSymbol:setAnchor(0.5, 0.5)
	textureSymbol:setScale(ISMap.SCALE*2.0)-- 3 seems to be a good multiplier. a range between 3-6 is ok, depending on how crisp you want the boundary to look. setting it lower may lead to gaps and might be harder to see.
end
