...
SandboxVars.BioGas = SandboxVars.BioGas or {}
SandboxVars.BioGas.spawnIndustrialTanks = false
...
local function OnPostDistributionMergeSD5()
			...
			elseif containerInfo.type == "junk" then
			  for i = 1, #containerData.junk.items, 2 do
				if containerData.junk.items[i] == itemName then
				  ...
				  if multiplier == 0 then
				    containerData.junk.items[i+1] = 0
				  else
	...
	for i=1,#japaneseClothing do
		if ZombRand(5) == 0 then
			modifyItemWeight(japaneseClothing[i], 0.1)
		else
			modifyItemWeight(japaneseClothing[i], 0)
		end
	end
	...
	--modifyItemWeight("TW.Motor", 0.75)
