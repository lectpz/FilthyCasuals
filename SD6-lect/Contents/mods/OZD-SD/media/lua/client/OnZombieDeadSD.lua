...
-- calculate roll for tiers /// ZombRand(n) where n is the number of zombies to kill before that tier may drop
	...
	local t5roll = ZombRand(math.max(SandboxVars.OZD.roll5 - math.floor(zombieSprinterValue*2.5), 1))
...
--	local tierzone = 4
	...
	if eventenabled then
		...
		if x >= x1 and y >= y1 and x <= x2 and y <= y2 then
			...
			t5roll = ZombRand(math.max(SandboxVars.SDevents.roll5 - math.floor(zombieSprinterValue*2.5), 1))
		end
	end
